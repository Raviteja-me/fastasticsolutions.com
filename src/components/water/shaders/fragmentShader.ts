...
void main() {
  ...
  vec3 brightPink = vec3(0.8, 0.2, 0.6);
  vec3 accent = vec3(0.4, 0.1, 0.5);
  
  // Dynamic color mixing with more contrast
  float heightFactor = vPosition.z * 2.0 + 0.5;
  vec3 baseColor = mix(deepPurple, accent, heightFactor);
  baseColor = mix(baseColor, brightPink, ripple * 0.8);
  
  // Enhanced shimmer effect
  float shimmer = sin(uv.x * 30.0 + uv.y * 20.0 + uTime * 2.0) * 0.08;
  baseColor += shimmer * vec3(0.3, 0.1, 0.2);
  
  // Brighter highlights for visibility
  float highlight = pow(1.0 - dist, 4.0) * 0.4;
  baseColor += highlight * brightPink;
